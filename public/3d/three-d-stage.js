...
(() => {
  ...
  class ThreeDStage extends HTMLElement {
    ...
    async _boot() {
      ...
      // Clear to the stage background rather than leaving the canvas
      // transparent: a transparent canvas relies on the :host background
      // bleeding through, which screenshots and PDF exports composite over
      // white instead.
      renderer.setClearColor(new THREE.Color(bg || '#f0eee6'), 1);
      ...
      // Bench lighting per the ISOLATION spec: warm ~4200 K key from the
      // upper-left, cool #79D2E6 fill at low intensity from the lower-right,
      // and a warm #F2A93B rim. Plus a procedural environment so metalness
      // 1.0 materials (copper, ENIG) have something to reflect.
      scene.add(new THREE.HemisphereLight(0xdfe7ee, 0x1a1d22, 0.45));
      const key = new THREE.DirectionalLight(0xffe6cc, 2.0);
      ...
      this._key = key;
      scene.add(key);
      const fill = new THREE.DirectionalLight(0x79d2e6, 0.5);
      fill.position.set(5, -2, -4);
      scene.add(fill);
      const rim = new THREE.DirectionalLight(0xf2a93b, 0.28);
      rim.position.set(-3, 1.5, -6);
      scene.add(rim);
      ...
      const panel = (hex, intensity, w, h, pos, look) => {
        const p = new THREE.Mesh(new THREE.PlaneGeometry(w, h), lit(hex, intensity));
        p.position.set(...pos);
        p.lookAt(...(look || [0, 0, 0]));
        envScene.add(p);
      };
      ...
      // Metal at metalness 1.0 has no diffuse term, so it takes these panels'
      // hue almost unmodified. The cool fill must stay well below the key or
      // neutral steel and aluminium read as tinted plastic.
      panel(0xf3ece2, 3.0, 22, 14, [-14, 16, 10]);   // key softbox, upper-left
      panel(0x79d2e6, 0.30, 20, 12, [15, -4, -10]);  // cool fill, lower-right
      panel(0xf2a93b, 0.22, 16, 10, [-8, 3, -18]);   // warm rim, behind
      panel(0x2a3038, 0.9, 40, 40, [0, -14, 0], [0, 1, 0]); // bench surface bounce
      ...
      this._loop = () => {
        // ResizeObserver and rAF both ride the document's rendering steps,
        // so a document that isn't painting at load can latch the canvas at
        // 1×1 with no recovery. Re-check every frame instead of trusting it.
        fit();
        controls.update();
